...
def get_info():
    data = request.json
    url = data.get('url')
    if not url or url == 'undefined': return jsonify({'error': 'No URL provided'}), 400
    try:
        with yt_dlp.YoutubeDL({'quiet': True}) as ydl:
            info = ydl.extract_info(url, download=False)
            formats = [{'id':'mp3','label':'Audio Only (MP3)'},
...
def download_video():
    ...
    tasks[file_id] = {'id': file_id,'status': 'pending','progress': '0',
